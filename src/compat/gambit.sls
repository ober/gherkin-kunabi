...
(library (compat gambit)
  ...
  ;; waitpid stubs (gherkin-lsp doesn't need process management)
  (define (c-do-waitpid pid flags) 0)
  (define (c-get-waitpid-status) 0)
  ...
  ;; Terminal control stubs (gherkin-lsp doesn't need terminal mode)
  (define (c-ffi-set-raw-mode fd) 0)
  (define (c-ffi-termios-restore fd flags) 0)
  ...
  ;; open(2) via foreign-procedure for /dev/fd/N paths
  ...
  ;; --- Crypto ---
  ;; SHA256 via OpenSSL CLI (stub until proper FFI binding)
  (define (sha256 data)
    (let* ((bv (if (string? data) (string->utf8 data) data))
           (p (open-process-ports "openssl dgst -sha256 -binary" 'block (native-transcoder)))
           (out (cadr p))   ;; stdout (input port)
           (in (caddr p))   ;; stdin (output port)
           (err (cadddr p)));; stderr
      (put-bytevector in bv)
      (close-port in)
      (let ((result (get-bytevector-all out)))
        (close-port out)
        (close-port err)
        result)))
